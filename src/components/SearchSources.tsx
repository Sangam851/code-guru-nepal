...
export function SourceCards({ sources }: { sources: AnswerSource[] }) {
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
        {sources.map((s, i) => (
          <a
            ...
            className="shrink-0 w-[168px] rounded-xl border border-border/60 bg-card/60 hover:bg-card px-2.5 py-2 transition"
          >
    ...
    <div className="flex flex-wrap gap-1.5 pt-0.5">
      {questions.map((q) => (
        <button
          ...
          disabled={disabled}
          onClick={() => onPick(q)}
          className="rounded-full border border-border/60 bg-card/50 px-3 py-1 text-[11px] text-muted-foreground hover:text-foreground hover:border-primary/60 transition disabled:opacity-50"
        >
          {q}
        </button>
      ))}
